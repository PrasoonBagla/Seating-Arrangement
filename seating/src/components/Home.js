...
const Home = () => {
    ...
    const downloadroomwise = async () => {
        ...
        axios.post('http://localhost:9000/downloadroomwise', ispressed, { responseType: 'blob' })
            .then(response => {
                const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'Room_Wise.xlsx'); // Specify the file name
    ...
    const downloadcoursewise = async () => {
        const ispressed = {uniqueDates,uniqueTimes};
        axios.post('http://localhost:9000/downloadcoursewise', ispressed, { responseType: 'blob' })
            .then(response => {
                const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'courses_to_rooms.xlsx'); // Specify the file name
            document.body.appendChild(link);
            link.click();
            link.parentNode.removeChild(link);
            window.URL.revokeObjectURL(url);
    ...
    const matrixwise = async () => {
        const ispressed = {uniqueDates,uniqueTimes};
        axios.post('http://localhost:9000/matrixwise', ispressed, { responseType: 'blob' })
            .then(response => {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'Matrix_Wise.xlsx'); // Specify the file name
                document.body.appendChild(link);
                link.click();
                // Clean up
                link.parentNode.removeChild(link);
